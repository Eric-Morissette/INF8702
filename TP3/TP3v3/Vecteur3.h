...
namespace Math3D
{
    ...
    ///
    ///  @param [in]       Incident const Math3D::CVecteur3 &   Le vecteur à réfracter
    ///  @param [in]       Normale const Math3D::CVecteur3 &    La normale
    ...
    ///
    ///////////////////////////////////////////////////////////////////////////////
    inline const CVecteur3 CVecteur3::Refract( const CVecteur3& Incident, const CVecteur3& Normale, const REAL IndiceRefractionRatio )
    {
        // Selon les notes de cours chapitre 5

        // Parallele    = (n1/n2)*(Incident - (Incident.Normale)*Normale)
        // Racine       = 1-Norme(Parallele)^2
        // Result       = Parallele - (Normale*sqrt(Racine))
        CVecteur3 Parallele = IndiceRefractionRatio*(Incident - CVecteur3::ProdScal(Incident, Normale)*Normale);
        REAL Racine = RENDRE_REEL(1.0) - pow(CVecteur3::Norme(Parallele), 2);
        if (Racine > 0)
        {
            return Parallele - (sqrt(Racine)*Normale);
        }
        else // Racine negative, il y a un probleme dans le calcul
        {
			return CVecteur3::Reflect(Incident, Normale);
        }
    }
}
